...
export default function DashboardPage() {
  ...
  return (
    <div className="relative flex flex-col flex-1 p-6 lg:p-8 max-w-[1400px] mx-auto w-full">

      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <header className="mb-8 animate-section animate-section-1">
        <div className="relative">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">Visão geral dos seus projetos criados com IA.</p>
          </div>
        </div>
      </header>

      {/* ── Métricas ─────────────────────────────────────────────────────────── */}
      <div className="grid gap-4 md:grid-cols-3 mb-8 animate-section animate-section-2">
        <GradientMetricCard
      ...
      {/* ── Projetos Recentes ─────────────────────────────────────────────────── */}
      <section className="mb-8 animate-section animate-section-3">
        <div className="flex items-center justify-between mb-4">
          ...
          {hasProjects && (
            <Link href="/projects" className="flex items-center gap-1 text-xs font-medium text-primary/70 hover:text-primary transition-colors">
        ...
        {hasProjects ? (
              ...
              return (
                <Link key={p.id} href={`/create?projectId=${p.id}`}>
                  <div className={cn(
                    ...
                    'hover:border-primary/30 hover:shadow-[0_8px_32px_hsl(var(--primary)/0.14)] hover:-translate-y-0.5',
                    ...
                    <div className="relative h-36 bg-muted overflow-hidden">
                      {thumb ? (
                        ...
                        <div className={cn('w-full h-full flex items-center justify-center bg-gradient-to-br', cfg.gradient)}>
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{relativeDate(p.updated_at)}</p>
                    </div>
                  </div>
                </Link>
              )
          ...
          <div className="rounded-2xl border border-dashed border-border bg-card/50 p-14 flex flex-col items-center text-center gap-4">
            <div className="size-14 rounded-2xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg,#8b5cf620,#ec489910)' }}>
            ...
            <Link href="/create">
              <button
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-xs font-semibold hover:opacity-90 transition-all"
      ...
      {/* ── Formatos ─────────────────────────────────────────────────────────── */}
      <section className="mb-12 animate-section animate-section-4">
        <div className="flex items-center gap-2 mb-4">
          <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Formatos disponíveis</h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {ALL_FORMATS.map(fmt => {
            ...
            const Icon  = cfg.icon
            return (
              <Link key={fmt} href={`/create?format=${fmt}`}>
                <div className={cn(
                  ...
                  <div className={cn('w-11 h-11 rounded-xl flex items-center justify-center bg-gradient-to-br', cfg.iconBg, 'shadow-sm group-hover:scale-110 group-hover:shadow-lg transition-all duration-200')}>
                  ...
                  <div>
                    ...
                    {count > 0 && <p className="text-[11px] text-primary/60 mt-1">{count}× criado{count !== 1 ? 's' : ''}</p>}
                  </div>
                </div>
              </Link>
            )
...
function GradientMetricCard({
  ...
  return (
    <div className={cn('rounded-2xl p-6 relative overflow-hidden bg-gradient-to-br', gradient, shadow)}>
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="p-1.5 rounded-lg bg-white/15">{icon}</div>
        </div>
        <p className={cn('font-bold text-white tracking-tight', isText ? 'text-2xl' : 'text-4xl')}>{value}</p>
      </div>
    </div>
  )
}
